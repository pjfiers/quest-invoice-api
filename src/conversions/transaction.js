...
let convert = function (input, options, filename) {
    ...
    toSkip.forEach(paymentMethod => {
        if (_.lowerCase(input.payment_method) == paymentMethod) {
    ...
    line['boekjaar'] = moment(moment(input.applied_at)).add(3, 'M').format('YYYY')
    line['periode'] = moment(moment(input.applied_at)).add(3, 'M').format('M')
    line['boekstuknummer'] = null //input.number
    line['valuta'] = 'EUR'
    line['wisselkoers'] = null
    line['boekingsdatum'] = input.applied_at
    ...
    line['bedrag'] = input.payment_amount
    line['aantal'] = null
